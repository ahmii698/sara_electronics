...
import { Users, Package, DollarSign, TrendingUp, BarChart, LineChart, PieChart, Activity, Award, AlertTriangle } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // Admin ko selected branch set karo
  ...
  // ===== COMBINED DATA FOR ADMIN (All Branches) =====
  const combinedData = {
    name: 'All Branches',
    customers: 1284,
    products: 3456,
      ...
      return (
        <div className="chart-line-container">
          ...
          <div className="chart-legend">
            <span><span className="legend-dot gold"></span> {data.name}</span>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="chart-pie-container">
          <div className="pie-chart">
            <svg viewBox="0 0 220 220">
              {data.chartData.values.map((val, index) => {
                ...
                const colors = ['#C9A84C', '#1A2A4A', '#E8D5A3', '#4A3520', '#8B7355'];
                return (
                  <circle
                    key={index}
                    cx="110" cy="110" r="85"
          ...
          <div className="chart-legend pie-legend">
            {data.chartData.labels.map((label, index) => (
              <span key={index}>
                <span className="legend-dot" style={{ background: ['#C9A84C', '#1A2A4A', '#E8D5A3', '#4A3520', '#8B7355'][index % 5] }}></span>
                {label}
              </span>
            ))}
          </div>
  ...
  // ===== STATS =====
  const stats = [
    { label: 'Total Customers', value: data.customers.toLocaleString(), icon: Users, color: '#C9A84C' },
    { label: 'Products in Stock', value: data.products.toLocaleString(), icon: Package, color: '#1A2A4A' },
    { label: 'Revenue', value: `PKR ${(data.revenue).toLocaleString()}`, icon: DollarSign, color: '#0A1628' },
    { label: 'Recovery Rate', value: `${data.recoveryRate}%`, icon: TrendingUp, color: '#C9A84C' },
  ];
    ...
    <div className="dashboard-container">
      <div className="dashboard-header">
        <div className="header-left">
          <h2>Dashboard</h2>
          <p className="branch-label">📍 {data.name}</p>
        </div>
        {isAdmin && (
          <select 
        ...
        {isManager && userBranch && (
          <span className="manager-badge">📍 Branch {userBranch}</span>
        )}
      </div>
      ...
      <div className="performers-section">
        ...
        <div className="performers-grid">
          <div className="performer-card branch-1-card">
            <h4>{data.name}</h4>
      ...
      {/* ===== RECENT TRANSACTIONS & BRANCH OVERVIEW ===== */}
          ...
          <div className="branch-overview">
            {selectedBranch === 'all' ? (
              ...
              <div className="branch-row">
                <span>{data.name}</span>
                <div className="bar-track">
                  <div className="bar-fill gold" style={{ width: '100%' }}></div>
                </div>
